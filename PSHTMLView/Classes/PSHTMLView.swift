...
struct PSHTMLViewScripts {
    ...
    private static let heigthOnLoadScriptString = "window.onload= function () {window.webkit.messageHandlers.\(PSHTMLViewScriptMessage.HandlerName.onContentHeightChange.rawValue).postMessage({justLoaded:true,height: document.body.scrollHeight});};"
    private static let heigthOnResizeScriptString = "function incrementCounter() {window.webkit.messageHandlers.\(PSHTMLViewScriptMessage.HandlerName.onContentHeightChange.rawValue).postMessage({height: document.body.scrollHeight});}; document.body.onresize = incrementCounter;"
    
    static let getContentHeightScriptString = "document.body.scrollHeight"
